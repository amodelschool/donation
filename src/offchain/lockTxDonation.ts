import { Address, DataB, Tx, Value } from '@harmoniclabs/plu-ts';
import { BrowserWallet } from '@meshsdk/core';
import getTxBuilder from './getTxBuilder';
import koios from './koios';
import { toPlutsUtxo } from './mesh-utils';
import { scriptAddr } from '../../contracts/conditionaldonation';
...
async function getLockTx(wallet: BrowserWallet, amount: string): Promise<Tx> {
	...
	const utxo = myUTxOs.find(
		(u) => u.resolved.value.lovelaces > amountLovelace
	);
...
export async function lockTx(
	...
	const unsignedTx = await getLockTx(wallet, amount);
