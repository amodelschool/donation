...
import { script, scriptAddr } from '../../contracts/conditionaldonation';

async function getUnlockTx( wallet: BrowserWallet, fulfilled: boolean ): Promise<Tx>
{
	...
	const utxoToSpend = (await koios.address.utxos( scriptAddr ))
	.find( utxo => {
	...
	const redeemer = new DataB( fromAscii(fulfilled ? 'true' : 'false') );
		...
		inputs: [
			{
				...
				inputScript: {
					...
					redeemer: redeemer, // match the redeemer
				}
			}
		],
...
export async function unlockTx( wallet: BrowserWallet, fulfilled: boolean ): Promise<string>
{
	const unsingedTx = await getUnlockTx( wallet, fulfilled );
